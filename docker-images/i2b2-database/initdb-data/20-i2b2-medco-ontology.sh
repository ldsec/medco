#!/bin/bash
set -e
# load the base structure of the i2b2-medco ontology
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" -d "$I2B2_MEDCO_DB_NAME" <<-EOSQL
    ...
    insert into i2b2metadata.schemes(c_key, c_name, c_description) values('TAG_ID:', 'TAG_ID', 'MedCo tag identifier');
    insert into i2b2metadata.schemes(c_key, c_name, c_description) values('ENC_ID:', 'ENC_ID', 'MedCo sensitive concept identifier (to be encrypted)');
    insert into i2b2metadata.schemes(c_key, c_name, c_description) values('CLEAR:', 'CLEAR', 'MedCo clear value');
    insert into i2b2metadata.schemes(c_key, c_name, c_description) values('GEN:', 'GEN', 'MedCo genomic annotations');
