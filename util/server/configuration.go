...
// GaDBHost is the host of the genomic annotations DB
var GaDBHost string

// GaDBPort is the number of the port used by the genomic annotations DB
var GaDBPort int

// GaDBName is the name of the genomic annotations database
var GaDBName string

// GaDBLoginUser is the genomic annotations database login user
var GaDBLoginUser string

// GaDBLoginPassword is the genomic annotations database login password
var GaDBLoginPassword string

// GaDBConnection is the connection to the genomic annotations database
var GaDBConnection *sql.DB
...
func init() {
	...
	GaDBHost = os.Getenv("GA_DB_HOST")
	GaDBName = os.Getenv("GA_DB_NAME")
	GaDBLoginUser = os.Getenv("GA_DB_USER")
	GaDBLoginPassword = os.Getenv("GA_DB_PW")

	dbPort, err := strconv.ParseInt(os.Getenv("GA_DB_PORT"), 10, 64)
	if err != nil || dbPort < 0 || dbPort > 65535 {
		logrus.Warn("invalid genomic annotations DB port, defaulted")
		dbPort = 5432
	}
	GaDBPort = int(dbPort)

	GaDBConnection, err = InitializeConnectionToDB(GaDBHost, GaDBPort, GaDBName, GaDBLoginUser, GaDBLoginPassword)
	if err != nil {
		logrus.Error("Impossible to initialize connection to genomic annotations DB")
		return
	}
	...
	if err != nil || dbPort < 0 || dbPort > 65535 {
		logrus.Warn("invalid i2b2 DB port, defaulted")
		dbPort = 5432
	}
