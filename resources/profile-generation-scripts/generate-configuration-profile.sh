...
# usage: bash generate-dev-configuration-profile.sh CONFIGURATION_PROFILE NODE_DNS_1 NODE_DNS_2 NODE_DNS_3 ...
if [[ $# -lt 3 ]]
then
    echo "Wrong number of arguments, usage: bash generate-configuration-profile.sh CONFIGURATION_PROFILE NODE_DNS_1 NODE_DNS_2 NODE_DNS_3 ..."
    exit
fi
...
"$CONF_FOLDER"/unlynxMedCo server generateTaggingSecrets --file "$CONF_FOLDER/group.toml"
