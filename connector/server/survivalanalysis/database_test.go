...
func TestBuildTimePoint(t *testing.T) {
	...
	timePoints, negativePatientList, err := buildTimePoints(
		bigList,
		[]string{`A168`},
		[]string{"@"},
		true,
	...
	assert.ElementsMatch(t, bigTimePoints, timePoints)
	assert.Empty(t, negativePatientList)

}

func TestBuildTimePointWithWrongParameters(t *testing.T) {
	utilserver.TestI2B2DBConnection(t)
	// reversing the start and end event
	// this is not expected to throw any error, simply empty results and a full negative list
	...
	assert.NoError(t, err)
	assert.Empty(t, timePoints)

	assert.Subset(t, bigList, negativePatientList)
