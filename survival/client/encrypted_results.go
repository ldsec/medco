...
func (ciphers EncryptedResults) Decrypt(privateKey string) (ClearResults, error) {
	...
	for i, cipher := range ciphers {
		...
		res[i].TimePoints = make(survivalcommon.TimePoints, len(cipher.TimePoints))
...
func encryptedResultsFromAPIResponse(bodyResults []*survival_analysis.ResultsItems0) EncryptedResults {
