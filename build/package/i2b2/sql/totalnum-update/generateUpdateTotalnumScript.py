...
-- set client_min_messages to 'info';
  ...
  * 	modifier totalnum = #(set of {subject}s directly referencing the modifier UNION of all set of {subject}s referencing the children of the modifier )
  *		
  * 	concept totalnum = #(set of {subject}s directly referencing the modifier UNION of all set of {subject}s referencing the children of the concept
  * 
  * Rules:
  ...
  * 3) A concept cannot have parents.
  * 
  * Hence one may see multiple layers of modifiers but there is a actually only one layer of concepts. 
  * 
 ...
 * At each iteration the code adds the {subject}s that relate to the child concept/modifiers to the set of {subject}s that relate to the parent.
...
CREATE OR REPLACE FUNCTION aggregateModifiersCounts() RETURNS void AS $$
	DECLARE
		min_depth int := (SELECT MIN(c_hlevel) FROM {metadata_schema_name}.{metadata_table_name} s WHERE s.c_facttablecolumn = 'MODIFIER_CD');
		max_depth int := (SELECT MAX(c_hlevel) FROM {metadata_schema_name}.{metadata_table_name} s WHERE s.c_facttablecolumn = 'MODIFIER_CD');
	BEGIN 
		...
		LOOP  
			...
			INSERT INTO {metadata_schema_name}.concept_to_id
				...
				WHERE cti.c_hlevel = height AND child.c_hlevel = height AND child.c_facttablecolumn = 'MODIFIER_CD' 
					AND cti.c_fullname = child.c_fullname AND 
					(
						( --the parent is a modifier at the height just above.
							parent.c_hlevel = (height - 1) AND parent.c_facttablecolumn = 'MODIFIER_CD' AND 
						...
						( --the parent is a concept
							parent.c_facttablecolumn = 'CONCEPT_CD' AND parent.c_fullname LIKE child.m_applied_path ESCAPE '|'
						)
					)
			; 
...
DO $$
BEGIN --TODO reestablish the commit
...
raise info 'After call to aggregate modifiers function.';
...
the totalnum column of your metadata table. We need a few informations about your database in order to generate that 
SQL script. We need the metadata schema name, that is the name of the schema that wraps your metadata table. The name 
of your metadata table which holds information about the modifiers and concepts such as the totalnum column. The owner 
of the temporary table that will be used in order to perform the count query. The data schema name, that is name of the schema
which contains your observation_fact table.
